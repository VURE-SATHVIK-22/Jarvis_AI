    ...
    if _PYGETWINDOW:
        try:
            ...
            if target_win:
                ...
                target_win.activate()
                time.sleep(1.0)
                print(f"[open_app] Focused existing window: '{target_win.title}'")
                return True
        except Exception as e:
            print(f"[open_app] ⚠️ PyGetWindow activation failed: {e}")
    ...
    if shutil.which(app_name) or shutil.which(app_name.split(".")[0]):
        try:
            subprocess.Popen(
                ...
                creationflags=subprocess.CREATE_NEW_PROCESS_GROUP if _SYSTEM == "Windows" else 0
            )
            time.sleep(1.5)
            return True
        except Exception as e:
